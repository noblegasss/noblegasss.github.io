...
suppose we have another $$\mathbf{X}_{\mathcal{M}'}$ \text{contains} $\rho_\mathcal{M}$$, 

such that 

$$\mathcal{C}(\mathbf{X}_{\mathcal{M}'}) = \mathcal{C}(\mathbf{X}_{\mathcal{M}})$$. 

Then $$\mathcal{J}_{n}\left(\hat{\eta}_{\mathcal{M}}\right)$$

is unique and positive definite for the fact that 
